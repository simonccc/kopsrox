...
def k3s_init_node(vmid: int = masterid,nodetype = 'master'):
  ...
  try:
    ...
    k3s_install_master = f'{k3s_install_version} sh -s - server --cluster-init --disable=servicelb --node-label="topology.kubernetes.io/region={cluster_name}" --node-label="topology.kubernetes.io/zone={proxmox_node}" {k3s_install_options}'
